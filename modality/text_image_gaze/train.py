...
def train_model(learning_rate, l2_penalty, epochs):
    ...
    for epoch in range(epochs):
        ...
        for contexts, contexts_len, images_path, gazes, responses, responses_len, labels in tqdm(train_loader):
            encoder.zero_grad()
        ...
        for contexts, contexts_len, images_path, gazes, responses, responses_len, labels in tqdm(val_loader):
            with torch.no_grad():
...
if text_model == 'lstm':
    ...
    encoder = TextImageGazeBertEncoder(image_model, synthesis_method, config)
encoder.to(device)
